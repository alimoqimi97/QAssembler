...
void Assembler::setErrors(QStringList &errors)
{
    this->Errors = errors;
}

QStringList Assembler::getErrors()
{
    return this->Errors;
}
...
void Assembler::InitializeCommandTypeTable()
{
    ...
    this->CommandTypeTable.insert("directive",directives);
}
...
//          inserting binary codes into machinecode     //
    ...
    else if(this->CommandTypeTable["I_Type"].contains(instruction))
    {
        ...
        //          finding errors          //
        if(this->ToBinary(fields.last()).size() > 15)
        {
            this->Errors.push_back("The size of offset is bigger than 15 bits: instruction = " + instruction);
            cerr << "The size of offset is bigger than 15 bits." << endl;
            return m;
        }
        //          --------------          //
        ...
        {
            I->setOffset(this->ToBinary(fields.last()));
        }
...
//        }
    ...
    else if(this->CommandTypeTable["directive"].contains(instruction))
    {
        this->Mcl.push_back(this->ToBinary(ac.getFields().first()));
        return m;
    }
    ...
    {
        this->Errors.push_back("Undifined Opcode(Command): " + instruction);
...
//    this means that the line of code is a directive command       //
    if(mc == nullptr)
    {
        return;
    }
...
{
    //      inserting labels in symboltable     //
    this->FirstScan(FileAddress);

    //      start processing on assemblycode    //
...
void Assembler::InsertLabelAddress()
{
    ...
    {
        this->Errors.push_back("Undifined label: " + label);
...
void Assembler::PrintMCL()
{
    ...
    if(this->Mcl.isEmpty())
    {
        cout << "The machine code list is empty..." << endl;
        return;
    }
